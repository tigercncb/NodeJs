...
MySpritePackTool.prototype.drawImages = function (obj) {
    ...
    if (obj.hasOwnProperty("assets")) {
        ...
        imgsInfo.forEach(item => {
            if (item.hasOwnProperty("atlas")) {
                ...
                imgObj.forEach(it => {
                    ...
                    let obj={
                        ...
                        "x": it["x"],
                        "y": it["y"],
                        "sourceW":this.options.maxSize.width,
                        "sourceH":this.options.maxSize.height,
                        "offX": 0,
			            "offY": 0,
                    }
                    showObj['frames'].push(obj)
                    showObj['file']=item['fileName']+".png"
                });
            ...
            showObj['count']=count
            let str=JSON.stringify(showObj)
            fs.writeFile(`${this.options.outPutPath}/${showObj['file']}.json`,str,function(error){
                if(error!=null)console.log("错误:"+error)
            })
        })
    }
